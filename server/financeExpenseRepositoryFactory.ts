...
import { deriveExpensePaymentBalance, deriveVendorBillBalance } from "./financeApDomainValidation";
...
export function createFinanceExpenseRepository(database: DrizzleDb): FinanceExpenseRepository {
  const repository: FinanceExpenseRepository = {
    transaction: async (work) => (
      typeof database.transaction === "function"
        ? await database.transaction(async (tx: DrizzleDb) => work(createFinanceExpenseRepository(tx)))
    ...
    listVendorBills: async (filters) => {
      ...
      const applications = await applicationsForBills(database, billIds);
      const documentCounts = await documentCountsForBills(database, billIds);
