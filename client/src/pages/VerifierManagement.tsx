...
export default function VerifierManagement() {
  ...
  const { data: allApprovals = [], isLoading: approvalsLoading } = useQuery<GuideApplicationApproval[]>({
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-4 items-center">
                ...
                <Select value={actionFilter} onValueChange={setActionFilter}>
                  <SelectTrigger className="max-w-xs" data-testid="select-action-filter">
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="require_more_info">Require More Info</SelectItem>
                  </SelectContent>
                </Select>
                <Select value={sortOrder} onValueChange={(value) => setSortOrder(value as "asc" | "desc")}>
                  <SelectTrigger className="max-w-xs" data-testid="select-sort-order">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              {approvalsLoading ? (
                ...
                <div className="space-y-3">
                  {allApprovals
                    .filter(approval => {
                      const matchesApplication = !historyFilter || 
                        approval.applicationId.toLowerCase().includes(historyFilter.toLowerCase());
                      const matchesAction = !actionFilter || approval.adminAction === actionFilter;
                    ...
                    .map((approval) => (
                      <Card key={approval.id} className="p-4">
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            <div className="flex items-center gap-3 mb-2">
                              <span className="font-medium text-sm" data-testid={`text-approval-app-id-${approval.id}`}>
                                App ID: {approval.applicationId}
                              </span>
                              {approval.adminAction && (
                                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                  approval.adminAction === 'approve' ? 'bg-green-500/10 text-green-700' :
                                  approval.adminAction === 'reject' ? 'bg-red-500/10 text-red-700' :
                                  approval.adminAction === 'require_more_info' ? 'bg-yellow-500/10 text-yellow-700' :
                            ...
                            <div className="grid grid-cols-2 gap-4 text-sm text-muted-foreground mb-2">
                              <div>
                                <span className="font-medium">User ID:</span> {approval.userId}
                              </div>
                              <div>
                                <span className="font-medium">Admin ID:</span> {approval.adminId || 'N/A'}
                              </div>
                              <div className="col-span-2">
                                <span className="font-medium">Date:</span> {new Date(approval.createdAt).toLocaleString()}
                              </div>
                            </div>
                            ...
                            {approval.userResponse && (
                              <div className="bg-blue-50 p-3 rounded-lg text-sm mt-2">
                                ...
                                <pre className="mt-1 whitespace-pre-wrap text-xs">
                                  {JSON.stringify(approval.userResponse, null, 2)}
                                </pre>
                              </div>
                            )}
                          </div>
                  ...
                  {allApprovals.length === 0 && (
                    <p className="text-muted-foreground text-center py-8" data-testid="text-no-approval-history">
